...
Partial Class Source_04_MOA04018
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            ...
            If LoginCheck.LoginCheck(user_id, "MOA04018") <> "" Then
                LoginCheck.LoginAction(Request.ServerVariables("REMOTE_ADDR"), user_id, "MOA04018.aspx")
                Response.End()
            End If
    ...
    Private Sub ImageButtonAdd_ClientScriptsPreparing()
        ...
        scripts.Append("javascript:return StringValidationAfterTrim({ controls: [ ") _
               ...
               .Append("blank_validation: [{ message: '請輸入牆柱編號' }] }, ") _
               .AppendFormat("{{ client_id: '{0}', ", tb_wa_name.ClientID) _
               .Append("blank_validation: [{ message: '請輸入牆柱名稱' }] }, ") _
    ...
    Protected Sub ibtnSearch_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ibtnSearch.Click
        ...
        tb_wa_code.Text = tb_wa_code.Text.Trim()
        tb_wa_name.Text = tb_wa_name.Text.Trim()
        tb_no.Text = tb_no.Text.Trim()

        Me.ViewState.Remove("wa_code")
        Me.ViewState.Remove("wa_name")
        Me.ViewState.Remove("no")

        If tb_wa_code.Text <> "" Then
            Me.ViewState("wa_code") = tb_wa_code.Text.Trim()
        End If

        If tb_wa_name.Text <> "" Then
            Me.ViewState("wa_name") = tb_wa_name.Text.Trim()
        End If

        If tb_no.Text <> "" Then
            Dim wa_no As Integer = 0
            Try
                wa_no = Integer.Parse(tb_no.Text.Trim())
            Catch ex As Exception
                ClientScript.RegisterStartupScript(Me.GetType(), "InputValueError", "alert('您欲查詢的牆柱排序順序資料不正確，請輸入數字!');", True)
            End Try
            If wa_no > 0 Then
                Me.ViewState("no") = wa_no
            End If
        End If
    ...
    Protected Sub ibtnAdd_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ibtnAdd.Click
        Dim wa_code As String = tb_wa_code.Text.Trim()
        Dim wa_name As String = tb_wa_name.Text.Trim()
        Dim wa_no As Integer = 0
        Try
            wa_no = Integer.Parse(tb_no.Text.Trim())
        Catch ex As Exception
            ClientScript.RegisterStartupScript(Me.GetType(), "InputValueError", "alert('您欲新增的牆柱排序順序資料不正確，請輸入數字!');", True)
        End Try
        ...
        If wa_no > 0 Then
            ...
            command.CommandText = "Insert Into P_0413(wa_code, wa_name, no) " & _
                                  "Values(@wa_code, @wa_name, @no)"

            command.Parameters.Add(New SqlParameter("wa_code", SqlDbType.VarChar, 1)).Value = wa_code
            command.Parameters.Add(New SqlParameter("wa_name", SqlDbType.VarChar, 50)).Value = wa_name
            command.Parameters.Add(New SqlParameter("no", SqlDbType.Int)).Value = wa_no
            ...
            End Try
        Else
            ClientScript.RegisterStartupScript(Me.GetType(), "InputValueError", "alert('您欲新增的牆柱排序順序資料不正確，請輸入大於0的數字!');", True)
        End If
    ...
    Private Sub GridView1_DataRebinding()
        ...
        If Not String.IsNullOrEmpty(Me.ViewState("wa_code")) Then

            sql_statement += " And wa_code = @wa_code"
            SqlDataSource1.SelectParameters.Add(New Parameter("wa_code", DbType.String, Me.ViewState("wa_code")))

        End If

        If Not String.IsNullOrEmpty(Me.ViewState("wa_name")) Then

            sql_statement += " And wa_name Like '%' + @wa_name + '%'"
            SqlDataSource1.SelectParameters.Add(New Parameter("wa_name", DbType.String, Me.ViewState("wa_name")))
        ...
        If e.Row.RowType.Equals(DataControlRowType.DataRow) Then
            ...
            If e.Row.RowState.Equals(DataControlRowState.Edit) Or e.Row.RowState.Equals(DataControlRowState.Alternate Or DataControlRowState.Edit) Then
                ...
                scripts.Append("javascript:return StringValidationAfterTrim({ controls: [ ") _
                       ...
                       .Append("blank_validation: [{ message: '請輸入牆柱名稱' }] }, ")
